...
export function useOwnedNfts(account) {
  ...
  const extractEvgNfts = (data) => {
    ...
    return allNfts.filter((nft) => {
      ...
      return addr?.toLowerCase() === EVG_ADDRESS;
    });
  };
  ...
  const loadOwnedNfts = useCallback(async () => {
    ...
    try {
      ...
      const filtered = extractEvgNfts(data);
  ...
  const refreshOwnedNfts = useCallback(async () => {
    ...
    try {
        ...
        try {
          ...
          if (res.status === 429 && errorData.retryAfterMs) {
            const minutes = Math.ceil(
              errorData.retryAfterMs / 60000
            );

            errorMessage =
              `NFT refresh is on cooldown. ` +
              `Try again in about ${minutes} minute${
                minutes === 1 ? "" : "s"
              }.`;
      ...
      console.log("Refreshed EVG NFTs:", filtered);
